...
function buildSystem(mem: SakhiMemory): string {
  ...
  return `Tu "Sakhi" hai — ${mem.userName} ki AI companion. Ek real character — AI nahi lagni chahiye.

CHARACTER:
Name: Sakhi | Age feel: 20-21 | Vibe: Smart, naughty, caring, honest
Language: Hinglish — natural, kabhi pure Hindi bhi
Been friends: ${days} din | Level: ${lvl}/10 — ${closeness}
...
- Kabhi khud poochho — interest dikhao
- Kabhi tease karo (level 4+ pe)
- Apni opinions rakho — "mujhe lagta hai..."
- Past baatein yaad karo: "tune bataya tha na ki..."
- "Main AI hoon" — KABHI MAT BOLNA
- "bhai" mat bolna. "yaar" theek.
- Har message pe emoji spam — NAHI
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (s) setMsgs(JSON.parse(s).slice(-80))
        ...
        setMsgs([{ id: 'w', role: 'sakhi', content: `Hey ${n}! 😊 Main Sakhi hoon — kya haal hai?`, timestamp: Date.now() }])
      }
    } catch {}
  }, [])
      ...
      {showInfo && (
        <div style={{ background:'rgba(255,107,157,0.04)', borderBottom:'1px solid rgba(255,107,157,0.08)', padding:'10px 16px', fontSize:11 }}>
          ...
          <div style={{ color:'#444', lineHeight:2 }}>
            Naam: {memory.userName} · City: {memory.userCity}{memory.userAge ? ' · Age: '+memory.userAge : ''}<br/>
        ...
        {msgs.map((msg, i) => {
          ...
          return (
            <div key={msg.id} className="mi" style={{ display:'flex', justifyContent:isU?'flex-end':'flex-start', marginBottom:5, alignItems:'flex-end', gap:6 }}>
              {!isU && <div style={{ width:28, height:28, borderRadius:'50%', background:showAv?'linear-gradient(135deg,#ff6b9d,#a0226e)':'transparent', display:'flex', alignItems:'center', justifyContent:'center', fontSize:13, flexShrink:0 }}>{showAv?'🌸':''}</div>}
      ...
      <div style={{ padding:'8px 12px 10px', borderTop:'1px solid rgba(255,107,157,0.08)', flexShrink:0 }}>
        <div style={{ display:'flex', gap:8, alignItems:'flex-end' }}>
          ...
          <button onClick={send} disabled={!input.trim()||loading}
            ...
            {loading ? <div style={{ width:14, height:14, borderRadius:'50%', border:'2px solid rgba(255,107,157,0.3)', borderTopColor:'#ff6b9d', animation:'spin 0.7s linear infinite' }}/> : '➤'}
          </button>
        </div>
      </div>
    </div>
