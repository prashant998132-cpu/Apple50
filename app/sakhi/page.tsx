...
' : '='}
        ...
        <button onClick={() => { if(!confirm('Delete chat?')) return; localStorage.removeItem('sakhi_msgs_v3'); setMsgs([{ id:'r'+Date.now(), role:'sakhi', content:'Fresh start! =
 Bata kya ho raha hai?', timestamp:Date.now() }]) }} style={{ background:'none', border:'none', color:'#222', fontSize:15, cursor:'pointer' }}></button>
      </div>
        ...
        {msgs.map((msg, i) => {
          ...
          return (
            <div key={msg.id} className="mi" style={{ display:'flex', justifyContent:isU?'flex-end':'flex-start', marginBottom:5, alignItems:'flex-end', gap:6 }}>
              {!isU && <div style={{ width:28, height:28, borderRadius:'50%', background:showAv?'linear-gradient(135deg,#ff6b9d,#a0226e)':'transparent', display:'flex', alignItems:'center', justifyContent:'center', fontSize:13, flexShrink:0 }}>{showAv?'<8':''}</div>}
