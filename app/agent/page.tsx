...
function AgentContent() {
  ...
  const start = async () => {
    ...
    await runAgent(
      ...
      (answer) => {
        // Save to history
        const run = { goal: goal.trim(), steps: steps.length, answer: answer.slice(0,300), ts: Date.now() }
  ...
  return (
    <div style={{ background: '#060610', height: '100dvh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', borderBottom: '1px solid #1e1e2e' }}>
        <button onClick={() => router.push('/')} style={{ background: 'none', border: 'none', color: '#666', fontSize: 20, cursor: 'pointer' }}>Ã¢ÂÂ</button>
        <div>
          <div style={{ color: '#00d4ff', fontWeight: 700, fontSize: 16 }}>Ã¢ÂÂ¡ Agent Mode</div>
        ...
        <div style={{ marginLeft: 'auto', padding: '3px 10px', borderRadius: 12, background: status === 'running' ? 'rgba(239,68,68,0.15)' : status === 'done' ? 'rgba(34,197,94,0.15)' : 'rgba(0,212,255,0.1)', color: status === 'running' ? '#ef4444' : status === 'done' ? '#22c55e' : '#00d4ff', fontSize: 10 }}>
          {status === 'idle' ? 'Ã¢ÂÂ Ready' : status === 'running' ? 'Ã¢ÂÂ Running' : status === 'done' ? 'Ã¢ÂÂ Done' : 'Ã¢ÂÂ Error'}
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 14, minHeight: 0 }}>
        ...
        {/* Idle state Ã¢ÂÂ show examples */}
        {/* Run History */}
        ...
        {status === 'idle' && steps.length === 0 && (
          <div>
            <div style={{ color: '#888', fontSize: 13, marginBottom: 14, lineHeight: 1.8 }}>
              Goal do Ã¢ÂÂ JARVIS khud plan banayega, tools use karega, aur complete karega. Tu sirf dekh!
            </div>
        ...
        {steps.map((step, i) => (
          <div key={i} style={{ marginBottom: 14 }}>
            ...
            <div style={{ background: 'rgba(0,212,255,0.05)', border: '1px solid rgba(0,212,255,0.1)', borderRadius: 10, padding: '8px 12px', marginBottom: 6 }}>
              <div style={{ color: '#555', fontSize: 10, marginBottom: 3 }}>Ã°ÂÂ§Â  THOUGHT {i+1}</div>
            ...
            <div style={{ background: '#0d0d18', border: '1px solid #1e1e2e', borderRadius: 10, padding: '8px 12px', marginBottom: 6 }}>
              <div style={{ color: '#555', fontSize: 10, marginBottom: 3 }}>Ã¢ÂÂ¡ ACTION</div>
            ...
            {step.result && (
              <div style={{ background: step.status === 'error' ? 'rgba(239,68,68,0.05)' : 'rgba(34,197,94,0.05)', border: `1px solid ${step.status === 'error' ? 'rgba(239,68,68,0.2)' : 'rgba(34,197,94,0.2)'}`, borderRadius: 10, padding: '8px 12px' }}>
                <div style={{ color: '#555', fontSize: 10, marginBottom: 3 }}>{step.status === 'error' ? 'Ã¢ÂÂ ERROR' : 'Ã¢ÂÂ RESULT'}</div>
            ...
            {step.status === 'running' && !step.result && (
              <div style={{ color: '#444', fontSize: 12, padding: '8px 12px' }}>Ã¢ÂÂ³ Running...</div>
            )}
          </div>
        ))}
        ...
        {finalAnswer && (
          <div style={{ background: 'rgba(0,212,255,0.08)', border: '1px solid rgba(0,212,255,0.3)', borderRadius: 12, padding: 14, marginTop: 8 }}>
            <div style={{ color: '#00d4ff', fontWeight: 700, fontSize: 13, marginBottom: 6 }}>Ã°ÂÂÂ¯ JARVIS Done!</div>
      ...
      <div style={{ padding: '10px 14px', borderTop: '1px solid #1e1e2e' }}>
        <div style={{ display: 'flex', gap: 8 }}>
          <input
            ...
            placeholder="Goal do Ã¢ÂÂ JARVIS khud karta hai..."
...
export default function AgentPage() {
  return (
    <Suspense fallback={<div style={{ background:'#060610', height:'100dvh', display:'flex', alignItems:'center', justifyContent:'center', color:'#00d4ff' }}>Ã¢ÂÂ³ Loading...</div>}>
      <AgentContent />
    </Suspense>
  )
}
