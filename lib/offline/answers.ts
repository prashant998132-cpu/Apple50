...
export const OFFLINE_ANSWERS: Record<string, string | (() => string)> = {
  ...
  // Tech
  'http status': '🌐 200=OK, 201=Created, 400=Bad Request, 401=Unauth, 403=Forbidden, 404=Not Found, 500=Server Error',
  'git commands': '📂 git init, git add ., git commit -m "msg", git push, git pull, git status, git log',
  'javascript': '💛 JS: let/const/var, arrow fn: ()=>{}, async/await, promises, DOM manipulation',
  'python': '🐍 Python: print(), input(), len(), range(), list/dict/tuple, def, class, import',
  'react': '⚛️ React: useState, useEffect, props, JSX, components, hooks, virtual DOM',
  'html tags': '🏷️ div, span, p, h1-h6, a, img, input, button, form, ul, li, table',
  'css': '🎨 display:flex/grid, position:absolute/fixed, margin/padding, border-radius, z-index',
  'sql': '🗄️ SELECT * FROM table WHERE col=val ORDER BY col LIMIT 10',
  'regex': '🔍 . = any, * = 0+, + = 1+, ? = 0-1, ^ = start, $ = end, [] = char class',
  'linux commands': '🐧 ls, cd, mkdir, rm, cp, mv, cat, grep, chmod, sudo, apt install',
  'api rest': '🔌 GET=read, POST=create, PUT=update, DELETE=remove, 201=created, 404=not found',
  'big o': '⏱️ O(1)=constant, O(n)=linear, O(n²)=quadratic, O(log n)=logarithmic',
